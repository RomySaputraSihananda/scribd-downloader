...
class ScribdDownloader {
  ...
  async execute(url, flag) {
    let fn;
    if (flag === scribdFlag.IMAGE || flag === "i") {
    ...
    if (url.match(scribdRegex.DOCUMENT)) {
      return await fn(
        `https://www.scribd.com/embeds/${
          scribdRegex.DOCUMENT.exec(url)[2]
        }/content`
      );
  ...
  async embeds_default(url) {
    ...
    if (m) {
      ...
      let first_page = await page.$(
        "div.outer_page_container div[id^='outer_page_']"
      );
      let style = await first_page.evaluate((el) => el.getAttribute("style"));
      if (style.includes("width:") && style.includes("height:")) {
        options.height = parseInt(
          style.split("height:")[1].split("px")[0].trim()
        );
        options.width = parseInt(
          style.split("width:")[1].split("px")[0].trim()
        );
      }
      ...
      await page.pdf({ ...options, timeout: 9000 });
  ...
  async embeds_image(url) {
    ...
    if (m) {
      ...
      for (let i = 0; i < doc_pages.length; i++) {
        ...
        let width = 1191;
        let height = 1684;
        let style = await doc_pages[i].evaluate((el) =>
          el.getAttribute("style")
        );
        if (style.includes("width:") && style.includes("height:")) {
          height = Math.ceil(
            (width *
              parseInt(style.split("height:")[1].split("px")[0].trim())) /
              parseInt(style.split("width:")[1].split("px")[0].trim())
          );
        }
      ...
      // generate pdf
      const pdfOutput = `${output}/${id}.pdf`;
      await pdfGenerator.generate(images, pdfOutput);
